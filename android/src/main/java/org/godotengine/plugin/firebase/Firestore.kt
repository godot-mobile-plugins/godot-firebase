...
class Firestore(private val plugin: FirebasePlugin) {
	...
	fun setDocument(document: FirestoreDocument, merge: Boolean = false) {
		...
		task.addOnSuccessListener {
			...
			plugin.emitGodotSignal(SIGNAL_DOCUMENT_WRITTEN, FirestoreDocument(collection, documentId).getRawData())
		}.addOnFailureListener { e ->
	...
	fun getCollection(collection: String) {
		...
		firestore.collection(collection).get()
			.addOnSuccessListener { querySnapshot ->
				...
				plugin.emitGodotSignal(SIGNAL_COLLECTION_QUERY_COMPLETED, FirestoreResult(collection,querySnapshot).getRawData())
			}
			.addOnFailureListener { e ->
		...
		firestore.collection(collection).document(documentId).get()
			.addOnSuccessListener { documentSnapshot ->
				if (documentSnapshot.exists()) {
					...
					plugin.emitGodotSignal(SIGNAL_DOCUMENT_QUERY_COMPLETED, FirestoreDocument(documentSnapshot).getRawData())
				} else {
	...
	fun updateDocument(document: FirestoreDocument) {
		...
		firestore.collection(collection).document(documentId).update(map)
			.addOnSuccessListener {
				...
				plugin.emitGodotSignal(SIGNAL_DOCUMENT_UPDATED, FirestoreDocument(collection, documentId).getRawData())
			}
			.addOnFailureListener { e ->
	...
	fun deleteDocument(collection: String, documentId: String) {
		...
		firestore.collection(collection).document(documentId).delete()
			.addOnSuccessListener {
				...
				plugin.emitGodotSignal(SIGNAL_DOCUMENT_DELETED, FirestoreDocument(collection, documentId).getRawData())
			}
			.addOnFailureListener { e ->
	...
	fun trackDocument(collection: String, documentId: String) {
	...
	fun stopTrackingDocument(collection: String, documentId: String) {
