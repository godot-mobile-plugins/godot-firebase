...
func _connect_signals():
	if not _plugin_singleton:
		return
	_plugin_singleton.connect(auth_success.get_name(), _on_auth_success)
	_plugin_singleton.connect(auth_failure.get_name(), auth_failure.emit)
	_plugin_singleton.connect(link_with_google_success.get_name(), _on_link_with_google_success)
	_plugin_singleton.connect(link_with_google_failure.get_name(), link_with_google_failure.emit)
	_plugin_singleton.connect(sign_out_success.get_name(), sign_out_success.emit)
	_plugin_singleton.connect(password_reset_sent.get_name(), password_reset_sent.emit)
	_plugin_singleton.connect(email_verification_sent.get_name(), email_verification_sent.emit)
	_plugin_singleton.connect(user_deleted.get_name(), user_deleted.emit)
...
func get_current_user() -> FirebaseUser:
	...
	if _plugin_singleton:
		user_data = _plugin_singleton.get_current_user()
	return null if user_data == null or user_data.is_empty() else FirebaseUser.new(user_data)
