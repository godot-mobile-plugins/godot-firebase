...
func _update_plugin() -> void:
	if _plugin_singleton == null:
		if Engine.has_singleton(PLUGIN_SINGLETON_NAME):
			...
			Firebase.log_error("%s singleton not found on this platform!" % PLUGIN_SINGLETON_NAME)
...
func get_firebase() -> Array:
	var __result: Array = []

	if _plugin_singleton:
		__result = _plugin_singleton.get_firebase()
	else:
		Firebase.log_error("%s plugin not initialized" % PLUGIN_SINGLETON_NAME)
