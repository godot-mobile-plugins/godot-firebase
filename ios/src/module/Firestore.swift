...
@objc final class Firestore: NSObject {
	...
	@objc func getDocument(_ collection: String, documentId: String) {
		backend.getDocument(
			...
			let document = FirestoreDocument(collection: collection, documentId: documentId, documentData: nil)
		...
		backend.getCollection(
			...
			if let error {
				self.emitter.emitCollectionQueryFailed(
					FirestoreError(collection: collection, documentId: nil, error: error.localizedDescription))
				return
			}
	...
	@objc func trackDocument(_ collection: String, documentId: String) {
		...
		let token = backend.addSnapshotListener(
			...
			if let error {
				self.emitter.emitDocumentQueryFailed(
					FirestoreError(collection: collection, documentId: documentId, error: error.localizedDescription))
				return
			}
